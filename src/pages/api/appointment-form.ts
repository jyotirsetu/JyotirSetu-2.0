...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const turnstileToken = (
      appointmentData['cf-turnstile-response'] ||
      appointmentData['turnstileToken'] ||
      ''
    ).toString();
      ...
      if (!verifyJson.success) {
        return new Response(
          JSON.stringify({
            ...
            debug: { stage: 'turnstile-verify', verifyJson },
          }),
    ...
    if (!parsed.success) {
      return new Response(
        JSON.stringify({
          ...
          debug: { stage: 'validation', issues: parsed.error.flatten() },
        }),
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (!hasUrl || !hasToken) {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          message: msg,
          debug: isDev ? { stage: 'env-check', hasUrl, hasToken } : undefined,
        }),
    ...
    try {
      ...
      return new Response(
        JSON.stringify({
          ...
          debug: isDev ? { stage: 'ensure-table', error: msg } : undefined,
        }),
        { status: 500, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    try {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          message: 'Failed to create appointment',
          debug: isDev ? { stage: 'client-init', error: msg } : undefined,
        }),
        { status: 500, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    try {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          message: 'Failed to create appointment',
          debug: isDev ? { stage: 'db-insert', error: msg } : undefined,
        }),
        { status: 500, headers: { 'Content-Type': 'application/json' } }
      );
    }
