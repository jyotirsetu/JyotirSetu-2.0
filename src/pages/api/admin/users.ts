...
async function assertSuperAdmin(request: Request){
  ...
  return requireRole(request, String(secret), ['super_admin'])
}
...
export const GET: APIRoute = async ({ request }) => {
  try {
    if (!(await assertSuperAdmin(request))) return new Response(JSON.stringify({ ok:false, error:'forbidden' }), { status:403 })
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    await db.execute({ sql:`INSERT INTO activity_log (id, action, entity_type, entity_id, user, details, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)`, args:['act_'+Date.now()+Math.random().toString(36).slice(2,8),'create','admin_user',id,'super_admin',JSON.stringify({ username, role }), now] })
...
export const PUT: APIRoute = async ({ request }) => {
  try {
    ...
    await db.execute({ sql:`INSERT INTO activity_log (id, action, entity_type, entity_id, user, details, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)`, args:['act_'+Date.now()+Math.random().toString(36).slice(2,8),'update','admin_user',id,'super_admin',JSON.stringify({ role, permissions }), now] })
