  ...
  try {
    ...
    if (priority) {
      ...
      await client.execute({
        sql: `UPDATE contacts SET priority = ? WHERE id = ?`,
        args: [String(priority), String(id)],
      });
  ...
  try {
    ...
    if (action === 'create') {
      if (!name || !email || !subject || !message) {
        return new Response(
          JSON.stringify({ ok: false, error: 'Missing required fields: name, email, subject, message' }),
          { status: 400 }
        );
      }
