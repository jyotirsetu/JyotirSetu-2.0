...
export const PUT: APIRoute = async ({ request }) => {
  try {
    ...
    if (!id || (!status && !payment_status)) return new Response(JSON.stringify({ ok: false, error: 'id and one of status/payment_status required' }), { status: 400 });
    await ensureAppointmentsTable();
    const client = await getTursoClient();
    const oldRes = await client.execute({ sql: `SELECT name, email, status, payment_status FROM appointments WHERE id = ?`, args: [String(id)] });
    const oldRow = (oldRes.rows?.[0] as unknown as { name: string, email: string, status: string, payment_status: string }) || { name: '', email: '', status: '', payment_status: '' };
    if (status) {
    ...
    if (payment_status) {
      const oldPay = oldRow.payment_status;
      await client.execute({ sql: `UPDATE appointments SET payment_status = ? WHERE id = ?`, args: [String(payment_status), String(id)] });
      await logActivity('payment_status_updated', 'appointment', id, `Payment changed from ${oldPay} to ${payment_status} for ${oldRow.name} (${oldRow.email})`);
    }
