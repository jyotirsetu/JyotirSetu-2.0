  ...
  try {
    ...
    const rows = (dataRes.rows as unknown as Appointment[]).map((r) => {
      ...
      if (r.service_details) {
        try {
          details = JSON.parse(r.service_details);
        } catch {
          details = String(r.service_details);
        }
      }
      return {
        ...r,
      ...
      } as Appointment & { service_details: unknown };
    });
...
export const PUT: APIRoute = async ({ request }) => {
  try {
    ...
    const { id, status, payment_status } = (body as { id?: string; status?: string; payment_status?: string }) || {};
    if (!id) return new Response(JSON.stringify({ ok: false, error: 'id required' }), { status: 400 });
    await ensureAppointmentsTable();
    const client = await getTursoClient();
    const oldRes = await client.execute({
      sql: `SELECT name, email, phone, service, date, time, consultation_method, status, payment_status FROM appointments WHERE id = ?`,
      args: [String(id)],
    });
    const oldRow = (oldRes.rows?.[0] as unknown as {
      name: string;
      email: string;
      phone: string;
      service: string;
    ...
    const updatable: (keyof AppointmentUpdateFields)[] = [
      'name',
      'email',
      'phone',
      'service',
    ...
    const updateBody = body as AppointmentUpdateFields;
    const entries = Object.entries(updateBody).filter(([k, v]) => (updatable as string[]).includes(k) && v != null);
    if (!entries.length)
      return new Response(JSON.stringify({ ok: false, error: 'no fields to update' }), { status: 400 });
    const setSql = entries.map(([k]) => `${k} = ?`).join(', ');
    const args = entries.map(([, v]) => String(v)) as (string | number | boolean | bigint | null)[];
    await client.execute({ sql: `UPDATE appointments SET ${setSql} WHERE id = ?`, args: [...args, String(id)] });
    if (status) {
      await logActivity(
        'status_updated',
        'appointment',
        id,
        `Status changed from ${oldRow.status} to ${status} for ${oldRow.name} (${oldRow.email})`
      );
    }
    if (payment_status) {
      await logActivity(
        ...
        'appointment',
        id,
        `Payment changed from ${oldRow.payment_status} to ${payment_status} for ${oldRow.name} (${oldRow.email})`
      );
    }
    if (
      updateBody.name ||
      ...
      updateBody.date ||
      updateBody.time ||
      updateBody.consultation_method
    ) {
      await logActivity(
        'appointment_updated',
        'appointment',
        id,
        `Appointment details updated for ${oldRow.name} (${oldRow.email})`
      );
    }
      ...
      if (!publicId) {
        ...
        try {
          await client.execute({
            sql: `UPDATE appointments SET customer_appointment_id = ? WHERE id = ?`,
            args: [publicId, String(id)],
          });
        } catch {
      ...
      try {
        ...
        if (lastSentAt && now - lastSentAt < recentWindowMs) {
          await logActivity(
            ...
            'appointment',
            id,
            `Duplicate email suppressed for ${row.name} (${row.email})`
          );
      ...
      if (html) {
        ok = await emailService.sendAppointmentCustomEmail(
          ...
          String(html),
          {
            ...
            new_date: String(new_date || row.date || ''),
            new_time: String(new_time || row.time || ''),
          }
        );
