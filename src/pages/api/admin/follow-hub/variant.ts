...
export const GET: APIRoute = async () => {
  try {
    ...
    const res = await client.execute({
      sql: `SELECT value FROM follow_hub_settings WHERE key='cta_variant' LIMIT 1`,
      args: [],
    });
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!['A', 'B'].includes(v)) {
      return new Response(JSON.stringify({ ok: false, error: 'invalid_variant' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    await client.execute({
      sql: `INSERT INTO follow_hub_settings (key, value, updated_at) VALUES ('cta_variant', ?, ?) ON CONFLICT(key) DO UPDATE SET value=excluded.value, updated_at=excluded.updated_at`,
      args: [v, now],
    });
