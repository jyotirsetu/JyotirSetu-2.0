...
export async function ensureAppointmentsTable() {
  ...
  try { await client.execute(`ALTER TABLE appointments ADD COLUMN payment_status TEXT NOT NULL DEFAULT 'unpaid'`); } catch (e) { console.warn('appointments: add payment_status column skipped', e); }
}
...
export async function ensurePaymentsTable() {
  ...
  try { await client.execute(`ALTER TABLE payments ADD COLUMN total_due REAL`); } catch (e) { console.warn('payments: add total_due column skipped', e); }
  try { await client.execute(`ALTER TABLE payments ADD COLUMN balance REAL`); } catch (e) { console.warn('payments: add balance column skipped', e); }
  try { await client.execute(`ALTER TABLE payments ADD COLUMN entry_type TEXT NOT NULL DEFAULT 'credit'`); } catch (e) { console.warn('payments: add entry_type column skipped', e); }
  try { await client.execute(`ALTER TABLE payments ADD COLUMN status TEXT NOT NULL DEFAULT 'unpaid'`); } catch (e) { console.warn('payments: add status column skipped', e); }
}
